...
class CropObservation(gym.Wrapper):
    """
    A wrapper that crops observations to a target size.
    ...
    Should be placed after color-space transforms but before SkipFrame:
        gym.make("CarRacing-v3") -> HSLObservation -> CropObservation -> SkipFrame
    ...
    def __init__(self, env, target_h=84, target_w=84):
        ...
        self.observation_space = Box(
            low=0, high=255, shape=(target_h, target_w, c), dtype=np.uint8
        )
